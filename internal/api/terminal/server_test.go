...
func startupGRPC() error {
	...
	pb.RegisterTerminalServer(server, &Server{})
...
func TestNew(t *testing.T) {
	stub, close, err := NewClient("localhost:8090")
	if err != nil {
		t.Fatal(err)
	}
	defer close()

	resp, err := stub.Create(context.Background(), &pb.Connection{
		Mode: pb.Connection_LOCAL,
		Ws: &pb.WinSize{
			Rows: 200,
			Cols: 100,
		},
